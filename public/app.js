...
(function () {
  ...
  const TTS_VOICES = [
    // Fish Audio 声线（语音克隆）
    { id: 'fish-7f92f8afb8ec43bf81429cc1c9199cb1', name: '🐟 AD学姐', desc: '⭐ Fish Audio — 御姐声线，清冷高傲', type: 'fish', referenceId: '7f92f8afb8ec43bf81429cc1c9199cb1' },
  ...
  function startCall() {
    ...
    try {
      ...
      // 保存衰减后的记忆
      localStorage.setItem(STORAGE.MEMORY.KEY_MEMORIES, JSON.stringify(decayedMemories));
  ...
  // ========== System Prompt 缓存（避免同一次 sendMessage 中重复构建） ==========
  let _promptDirty = false;  // 脏标记：是否需要重建 system prompt

  function markPromptDirty() { _promptDirty = true; }

  /**
   * 安全更新 System Prompt。使用脏标记批量更新，同一次 sendMessage 中多次调用只重建一次。
   */
  function updateSystemPrompt() {
    markPromptDirty();
  }

  /** 在 sendMessage 结束时调用，真正执行一次更新 */
  function flushSystemPrompt() {
  ...
  function buildSystemPrompt() {
    // 每个系统调用都加 try/catch，一个系统崩溃不影响整体
    let memoryContext = '';
    try { memoryContext = MemorySystem.buildMemoryContext(); } catch (e) { console.error('[Prompt] MemorySystem 构建失败:', e); }
    
    let affectionContext = '';
    try { affectionContext = AffectionSystem.buildAffectionContext(); } catch (e) { console.error('[Prompt] AffectionSystem 构建失败:', e); }
    
    let emotionalDepthContext = '';
    try { emotionalDepthContext = EmotionalDepthSystem.buildEmotionalContext(); } catch (e) { console.error('[Prompt] EmotionalDepthSystem 构建失败:', e); }
    
    let principlesContext = '';
    try { principlesContext = AIPrinciplesSystem.buildPrinciplesContext(); } catch (e) { console.error('[Prompt] AIPrinciplesSystem 构建失败:', e); }
    
    let personalityContext = '';
    try { personalityContext = PersonalityConsistencySystem.buildPersonalityContext(); } catch (e) { console.error('[Prompt] PersonalityConsistencySystem 构建失败:', e); }
...
✅ 好："诶？！真的吗...你、你真的要带我去吗...好感动... [surprised]"`;
    ...
    // 记忆不完美机制
    let mistakeHint = '';
    try { mistakeHint = MemorySystem.getMistakeHint(); } catch (e) { console.error('[Prompt] MemorySystem.getMistakeHint 失败:', e); }
    if (mistakeHint) {
      prompt += `\n\n${mistakeHint}`;
    }
...
`;
  ...
  async function sendMessage() {
    ...
    try {
      ...
      try {
        ...
        if (MemorySystem.detectCorrection(text)) {
          ...
          if (keyMemories.length > 0) {
            ...
            localStorage.setItem(STORAGE.MEMORY.KEY_MEMORIES, JSON.stringify(keyMemories));
      ...
      } else if (err.httpStatus === 504) {
        addChatMsg('ai', '（AI 服务响应超时，请稍后再试）');
      } else if (err.httpStatus === 401 || err.httpStatus === 403) {
        addChatMsg('ai', '（API Key 无效或已过期，请检查设置）');
      } else if (err.httpStatus === 429) {
        addChatMsg('ai', '（请求太频繁或额度不足，请稍后再试）');
      } else {
        addChatMsg('ai', `（网络错误，请检查 API 配置… ${err.message ? '[' + err.message + ']' : ''}）`);
      }
      setEmotion('idle');
    }
    ...
    // 批量刷新 System Prompt（同一次 sendMessage 中多次变更只重建一次）
    flushSystemPrompt();
  ...
  async function callAPI(overrideMessages) {
    ...
    try {
      ...
      if (!res.ok) {
        // 尽量读出后端返回的具体错误原因（超时/上游错误/连接失败等），
        // 而不是只抛一个 "HTTP 502" 让用户和排查者都不知道到底发生了什么。
        let detail = '';
        try { detail = (await res.json()).error || ''; } catch { /* 非 JSON 响应，忽略 */ }
    ...
    try {
      ...
      if (relationship.milestones && relationship.milestones.length > 0) {
        ...
        for (const milestone of recentMilestones) {
          ...
          item.innerHTML = `
            <div class="memory-text">${emotionEmoji} ${escapeHtml(milestone.title || '')}</div>
            <div class="memory-meta">${date} · ${escapeHtml(milestone.story || '')}</div>
          `;
      ...
      if (relationship.insideJokes && relationship.insideJokes.length > 0) {
        for (const joke of relationship.insideJokes) {
          ...
          item.innerHTML = `
            <div class="memory-text">😄 "${escapeHtml(joke.phrase || '')}"</div>
            <div class="memory-meta">${escapeHtml(joke.meaning || '')} · 用过 ${joke.timesUsed || 1} 次 · ${firstUsed}</div>
          `;
  ...
  function bindSettings() {
    ...
    $('#antiaddiction-reminder-enabled')?.addEventListener('change', (e) => {
      ...
      localStorage.setItem(STORAGE.ANTI_ADDICTION, JSON.stringify(state));
  ...
  function bindBackgrounds() {
    ...
    BG_PRESETS.forEach(bg => {
      const div = document.createElement('div');
      div.className = 'bg-thumb' + (config.sceneBg === bg.id ? ' active' : '');
  ...
  function saveConfig() {
    try {
      localStorage.setItem(STORAGE.CONFIG, JSON.stringify({
        ...
        ttsVoice: config.ttsVoice, background: config.sceneBg,
  ...
  function loadConfig() {
    try {
      ...
      if (saved) {
        ...
        // 迁移旧数据：旧版 background 存的是场景背景 ID（如 'bg-1'），
        // 新版 sceneBg 存场景背景，background 存人格深度文本
        if (parsed.background && !parsed.sceneBg && /^bg-\d+$/.test(parsed.background)) {
          parsed.sceneBg = parsed.background;
          parsed.background = '';
        }
