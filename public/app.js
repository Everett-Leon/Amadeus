...
`;
    ...
    try {
      ...
      try {
        ...
        if (unhealthyCheck.isUnhealthy) {
          ...
          $('#chat-messages').lastChild.querySelector('.msg-text').textContent = replacedText;
          $('#bubble-text').textContent = replacedText;
        }
        ...
        if (principlesUnhealthyCheck.isUnhealthy) {
          ...
          // 更新聊天历史和显示
          chatHistory[chatHistory.length - 1].content = replacedText;
          if ($('#chat-messages').lastChild) {
            const msgText = $('#chat-messages').lastChild.querySelector('.msg-text');
            if (msgText) msgText.textContent = replacedText;
          }
          if ($('#bubble-text')) $('#bubble-text').textContent = replacedText;
        }
  ...
  async function triggerProactiveMessage() {
    ...
    const proactiveSystemPrompt = `${chatHistory[0].content}\n\n【主动对话】你现在要主动找用户说话。用户已经有一段时间没说话了，你可以：\n- 问问用户在干什么\n- 分享你的想法\n- 表达你的情绪\n\n请自然地开启话题，不要太突兀。`;
