...
| 2 | 服务器偶发 502 | 中 → 部分改善 | `/api/chat` 之前把所有网络错误/超时/JSON解析失败都无差别标成 502，且完全没有服务端日志，根因根本无从查起。已加上 30s 超时 + `console.error` 详细日志 + 尽量透传上游真实状态码。**如果 502 再出现，现在能从终端日志看到真实原因**（超时/DNS失败/上游真实错误码），下次遇到可以直接看日志定位，而不是盲猜 |
| 3 | Safari 兼容 | 低（已放弃） | Safari 大量 CSS/JS 不兼容，建议用 Chrome 无痕模式 |
| 4 | F12 调试日志 | 已解决 | 排查后 `[Emotion Debug]`/`[BGM]` 这两个 tag 的日志在代码里已经不存在了（可能之前的清理已经做过），这条可以从清单划掉 |
| 5 | 蕾塞立绘素材 | 低 | 素材存在 `uploads/expressions/` 但未在代码中默认加载 |
| 6 | Unity 3D 集成 | Phase 3 | 前端稳定后再推 |
| 7 | 移动端适配 | Phase 4 | 远期目标 |
| 8 | Electron 打包 | 待定 | `electron.js` 已重构为直接 `require('./server.js')` 复用路由（之前是整份复制粘贴，这正是 Fish Audio Key 泄露的根源），语法检查 + 独立 `.listen()` 逻辑已验证通过；**仍未做完整 GUI 打包运行测试**，下次真要打包发布前建议先跑一次 `npm run electron` 实测 |
